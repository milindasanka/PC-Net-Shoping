...
if (isset($_GET["st"])) {
	...
	if ($p_st == "Completed") {
		...
		$sql = "SELECT product_qty FROM products WHERE product_id = 'p_id'";
		$query = mysqli_query($con,$sql);
		if (mysqli_num_rows($query) > 0) {
			...
			$nqty = $product_qty - $qty;

			$sql = "UPDATE products SET product_qty='$nqty' WHERE p_id = '$p_id' AND user_id = '$cm_user_id'";
			if (mysqli_query($con,$sql)) {
